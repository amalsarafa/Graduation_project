    ...
    <div class="page-content">
        ...
        <div class="page-bar">
            <ul class="page-breadcrumb">
                ...
                <li>
                    <a href="{{route('patient.reserve')}}"> احجز خدمة  </a>
                </li>
            </ul>
        </div>
        @if(session('success'))
...
@endif
            ...
            <div class="container">
                <h2 class="text-center">تعديل الحجز</h2>
                <hr>
                
                <div class="panel panel-default">
                    <div class="panel-heading">املأ المعلومات لتعديل حجز الخدمة</div>
                    <div class="panel-body">
                        <form action="{{ route('patient.reserve.update', $reservation->id) }}" method="POST">
                            @csrf
                            @method('PUT')
                                ...
                                <select class="form-control" name="doctor_user_id" required>
                                    ...
                                    @foreach($doctors as $doctor)
                                        <option value="{{ $doctor->id }}" 
                                                {{ $doctor->id == old('doctor_user_id', $reservation->doctor_user_id) ? 'selected' : '' }}>
                                            {{ $doctor->name }}
                                        </option>
                                    @endforeach
                                </select>
                            ...
                            <div class="form-group">
                                ...
                                <input type="time" class="form-control" name="time" value="{{ $reservation->time }}" required>
                            </div>
                            
                            <!-- الأزرار -->
                            <div class="form-actions">
                                <button type="submit" class="btn btn-success">
                                    <i class="fa fa-calendar-check-o"></i> تحديث الحجز
                                </button>
